...
次に、結果のエリアマークのいずれかの上部頂点を選択し、そのX位置を「date」にバインドします。下部頂点についても同様に行い、既存の日付スケールとマージすることを選択します。Data Illustrateur は頂点のX位置とエリアマークのX位置を区別しますが、ここでは「date」をエンコードするために前者を使用しています。
